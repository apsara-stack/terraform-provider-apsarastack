...
func init() {
	resource.AddTestSweepers("apsarastack_edas_k8s_cluster", &resource.Sweeper{
		Name: "apsarastack_edas_k8s_cluster",
...
func TestAccApsaraStackEdasK8sCluster_basic(t *testing.T) {
	var v *edas.Cluster
	resourceId := "apsarastack_edas_k8s_cluster.default"
...
func testAccCheckEdasK8sClusterDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "apsarastack_edas_k8s_cluster" {
			continue
		}
