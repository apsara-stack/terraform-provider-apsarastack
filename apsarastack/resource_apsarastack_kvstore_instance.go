...
func resourceApsaraStackKVStoreInstance() *schema.Resource {
	return &schema.Resource{
		Create: resourceApsaraStackKVStoreInstanceCreate,
		Read:   resourceApsaraStackKVStoreInstanceRead,
		Update: resourceApsaraStackKVStoreInstanceUpdate,
		Delete: resourceApsaraStackKVStoreInstanceDelete,
...
func resourceApsaraStackKVStoreInstanceCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if v, ok := d.GetOk("node_type"); ok && v.(string) != "" {
	...
	if v, ok := d.GetOk("architecture_type"); ok && v.(string) != "" {
		request.QueryParams["ArchitectureType"] = v.(string)
	}
	...
	if tde, ok := d.GetOk("tde_status"); ok && tde.(string) == "Enabled" {
		...
		tde_req.Headers = map[string]string{
			"RegionId":             client.RegionId,
			"x-ascm-product-name":  "R-kvstore",
...
func resourceApsaraStackKVStoreInstanceUpdate(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.ApsaraStackClient)
	kvstoreService := KvstoreService{client}
	d.Partial(true)
	...
	if d.IsNewResource() {
		d.Partial(false)
		return resourceApsaraStackKVStoreInstanceRead(d, meta)
	}
...
func resourceApsaraStackKVStoreInstanceDelete(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.ApsaraStackClient)
	kvstoreService := KvstoreService{client}
