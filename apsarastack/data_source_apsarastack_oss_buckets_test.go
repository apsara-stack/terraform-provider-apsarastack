...
func TestAccApsaraStackOssBucketsDataSource_basic(t *testing.T) {
	...
	resourceId := "data.apsarastack_oss_buckets.default"
	...
	nameRegexConf := dataSourceTestAccConfig{
		existConfig: testAccConfig(map[string]interface{}{
			"name_regex": "${apsarastack_oss_bucket.default.bucket}",
		}),
		fakeConfig: testAccConfig(map[string]interface{}{
			"name_regex": "${apsarastack_oss_bucket.default.bucket}-fake",
		}),
	}
	var existOssBucketsMapFunc = func(rand int) map[string]string {
		return map[string]string{
			...
			"buckets.0.referer_config.0.referers.0":  "http://www.apsarastack.com",
...
func TestAccApsaraStackOssBucketsDataSource_sserule(t *testing.T) {
	rand := acctest.RandIntRange(1000000, 9999999)
	resourceId := "data.apsarastack_oss_buckets.default"
...
func TestAccApsaraStackOssBucketsDataSource_versioning(t *testing.T) {
	rand := acctest.RandIntRange(1000000, 9999999)
	resourceId := "data.apsarastack_oss_buckets.default"
...
resource "apsarastack_oss_bucket" "default" {
	...
	logging {
		target_bucket = "${apsarastack_oss_bucket.target.id}"
		target_prefix = "log/"
	}

	referer_config {
		allow_empty = false
		referers = ["http://www.apsarastack.com"]
	}
