...
func init() {
	resource.AddTestSweepers("apsarastack_ess_scheduled_task", &resource.Sweeper{
		Name: "apsarastack_ess_scheduled_task",
...
func testSweepEssSchedules(region string) error {
	...
	req.QueryParams = map[string]string{"Product": "ess", "Department": client.Department, "ResourceGroup": client.ResourceGroup}
...
func TestAccApsaraStackEssScheduledTask_basic(t *testing.T) {
	var v ess.ScheduledTask
	resourceId := "apsarastack_ess_scheduled_task.default"
...
func TestAccApsaraStackEssScheduledTask_multi(t *testing.T) {
	var v ess.ScheduledTask
	resourceId := "apsarastack_ess_scheduled_task.default.9"
	ra := resourceAttrInit(resourceId, nil)
	rc := resourceCheckInit(resourceId, &v, func() interface{} {
		return &EssService{testAccProvider.Meta().(*connectivity.ApsaraStackClient)}
	})
...
func testAccCheckEssScheduledTaskDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "apsarastack_ess_scheduled_task" {
			continue
		}
