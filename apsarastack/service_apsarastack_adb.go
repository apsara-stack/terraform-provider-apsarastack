...
func (s *AdbService) DescribeDBSecurityIps(clusterId string) (ips []string, err error) {
	...
	if err != nil {
		return ips, WrapErrorf(err, DefaultErrorMsg, clusterId, request.GetActionName(), ApsaraStackSdkGoERROR)
	}
	addDebug(request.GetActionName(), raw, request.RpcRequest, request)
...
func (s *AdbService) ModifyDBSecurityIps(clusterId, ips string) error {
	...
	if err != nil {
		return WrapErrorf(err, DefaultErrorMsg, clusterId, request.GetActionName(), ApsaraStackSdkGoERROR)
	}
