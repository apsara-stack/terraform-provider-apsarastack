...
func TestAccApsaraStackDnsDomainsDataSource(t *testing.T) {
	rand := acctest.RandIntRange(1000, 9999)
	testAccConfig := dataSourceTestAccConfigFunc("data.apsarastack_dns_domains.default", strconv.FormatInt(int64(rand), 10), dataSourceDnsDomainsConfigDependence)
	aliDomainConf := dataSourceTestAccConfig{
		existConfig: testAccConfig(map[string]interface{}{
			"ali_domain":        "false",
			"domain_name_regex": "${apsarastack_dns.default.name}",
		}),
	...
	idsConf := dataSourceTestAccConfig{
		existConfig: testAccConfig(map[string]interface{}{
			"ids": []string{"${apsarastack_dns.default.domain_id}"},
		}),
		fakeConfig: testAccConfig(map[string]interface{}{
			"ids": []string{"${apsarastack_dns.default.domain_id}-fake"},
		}),
	}
	groupNameConf := dataSourceTestAccConfig{
		existConfig: testAccConfig(map[string]interface{}{
			"ali_domain":        "false",
			"group_name_regex":  "${apsarastack_dns_group.default.name}",
			"domain_name_regex": "${apsarastack_dns.default.name}",
		}),
		fakeConfig: testAccConfig(map[string]interface{}{
			"ali_domain":        "false",
			"group_name_regex":  "${apsarastack_dns_group.default.name}_fake",
			"domain_name_regex": "${apsarastack_dns.default.name}",
		}),
	}
...
`, defaultRegionToTest, name, name, os.Getenv("APSARASTACK_RESOURCE_GROUP_ID"))
}
...
var dnsDomainsCheckInfo = dataSourceAttr{
	resourceId:   "data.apsarastack_dns_domains.default",
