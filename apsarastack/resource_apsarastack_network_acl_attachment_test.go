...
func init() {
	resource.AddTestSweepers("apsarastack_network_acl_attachment", &resource.Sweeper{
		Name: "apsarastack_network_acl_attachment",
...
// Skip the test because 'resources' is conflict with 'apsarastack_network_acl'.
func TestAccApsaraStackVpcNetworkAclAttachment_basic(t *testing.T) {
	resourceId := "apsarastack_network_acl_attachment.default"
...
func testAccCheckNetworkAclAttachmentDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "apsarastack_network_acl_attachment" {
			continue
		}
...
resource "apsarastack_network_acl_attachment" "default" {
	...
	resources {
          resource_id = "${apsarastack_vswitch.default2.id}"
          resource_type = "VSwitch"
        }
}
`, randInt)
