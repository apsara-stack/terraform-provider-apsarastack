...
func resourceApsarastackEssAttachment() *schema.Resource {
	return &schema.Resource{
		Create: resourceApsarastackEssAttachmentCreate,
		Read:   resourceApsarastackEssAttachmentRead,
		Update: resourceApsarastackEssAttachmentUpdate,
		Delete: resourceApsarastackEssAttachmentDelete,
...
func resourceApsarastackEssAttachmentCreate(d *schema.ResourceData, meta interface{}) error {

	d.SetId(d.Get("scaling_group_id").(string))
	return resourceApsarastackEssAttachmentUpdate(d, meta)
}
...
func resourceApsarastackEssAttachmentUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	return resourceApsarastackEssAttachmentRead(d, meta)
}
...
func resourceApsarastackEssAttachmentDelete(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.ApsaraStackClient)
	essService := EssService{client}
	...
	if err := resource.Retry(5*time.Minute, func() *resource.RetryError {
		...
		if err != nil {
			...
			if IsExpectedErrors(err, []string{"ScalingActivityInProgress", "IncorrectScalingGroupStatus"}) {
				time.Sleep(5)
				return resource.RetryableError(WrapErrorf(err, DefaultErrorMsg, d.Id(), request.GetActionName(), ApsaraStackSdkGoERROR))
			}
