...
func init() {
	resource.AddTestSweepers("apsarastack_edas_cluster", &resource.Sweeper{
		Name: "apsarastack_edas_cluster",
		F:    testSweepEdasCluster,
		Dependencies: []string{
			"apsarastack_edas_application",
		},
	})
}
...
func TestAccApsaraStackEdasCluster_basic(t *testing.T) {
	var v *edas.Cluster
	resourceId := "apsarastack_edas_cluster.name1"
...
func TestAccApsaraStackEdasCluster_multi(t *testing.T) {
	var v *edas.Cluster
	resourceId := "apsarastack_edas_cluster.default.1"
	ra := resourceAttrInit(resourceId, edasClusterBasicMap)
	serviceFunc := func() interface{} {
		return &EdasService{testAccProvider.Meta().(*connectivity.ApsaraStackClient)}
	}
...
func testAccCheckEdasClusterDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "apsarastack_edas_cluster" {
			continue
		}
