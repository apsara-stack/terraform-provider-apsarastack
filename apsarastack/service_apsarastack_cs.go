...
func (s *CsService) DescribeClusterNodes(id, nodepoolid string) (pools *NodePools, err error) {
	...
	if err := invoker.Run(func() error {
		...
		return nil, WrapErrorf(err, DefaultErrorMsg, id, "DescribeClusterNodes", ApsaraStackSdkGoERROR)
	}
...
func (s *CsService) DescribeClusterNodePools(id string) (*NodePool, error) {
	...
	//return err
	if err != nil {
		return nil, WrapErrorf(err, DefaultErrorMsg, "apsarastack_cs_kubernetes", "CreateKubernetesCluster", raw)
	}
	var node *NodePool
	nodePool, _ := raw.(*responses.CommonResponse)
	if nodePool.IsSuccess() == false {
		return nil, WrapErrorf(err, DefaultErrorMsg, "apsarastack_ascm", "API Action", nodePool.GetHttpContentString())
	}
	ok := json.Unmarshal(nodePool.GetHttpContentBytes(), &node)
	if ok != nil {
		return nil, WrapErrorf(err, DefaultErrorMsg, "apsarastack_cs_kubernetes", "ParseKubernetesClusterResponse", raw)
	}
