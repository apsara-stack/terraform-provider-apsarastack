...
func resourceApsaraStackMaxcomputeProject() *schema.Resource {
	return &schema.Resource{
		...
		Update: resourceApsaraStackMaxcomputeProjectUpdate,
...
func resourceApsaraStackMaxcomputeProjectCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if cluster == nil {
		return WrapErrorf(err, DefaultErrorMsg, "apsarastack_maxcompute_cluster", "getCluster", cluster_name)
	}
	...
	if v, ok := d.GetOk("enabled_mc_encrypt"); ok && v.(bool) {
		...
		if _, ok := d.GetOk("mc_encrypt_algorithm"); !ok {
			...
			return WrapErrorf(err, DataDefaultErrorMsg, "apsarastack_maxcompute_project", "mc_encrypt_algorithm", ApsaraStackSdkGoERROR)
	...
	if err != nil {
		return WrapErrorf(err, DefaultErrorMsg, "apsarastack_maxcompute_project", "Create", raw)
	}
	...
	if bresponse.GetHttpStatus() != 200 {
		return WrapErrorf(err, DefaultErrorMsg, "apsarastack_maxcompute_project", "Create", ApsaraStackSdkGoERROR)
	}
...
func resourceApsaraStackMaxcomputeProjectUpdate(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.ApsaraStackClient)
	...
	if d.HasChange("cluster") || d.HasChange("disk") {
		...
		if err != nil {
			...
			return WrapErrorf(err, DefaultErrorMsg, d.Get("user_name").(string), "UpdateOdpsQuota", ApsaraStackSdkGoERROR)
