...
func init() {
	resource.AddTestSweepers("apsarastack_kms_key", &resource.Sweeper{
		Name: "apsarastack_kms_key",
		F:    testSweepKmsKey,
	})
}
...
func testSweepKmsKey(region string) error {
	...
	if err != nil {
		return WrapErrorf(err, DataDefaultErrorMsg, "kms_keys", req.GetActionName(), ApsaraStackSdkGoERROR)
	}
	...
	for _, v := range keys.Keys.Key {
		...
		for _, description := range prefixes {
			if strings.HasPrefix(strings.ToLower(key.Description), strings.ToLower(description)) {
				...
				swept = true
				if err != nil {
					return WrapErrorf(err, DataDefaultErrorMsg, v.KeyId, req.GetActionName(), ApsaraStackSdkGoERROR)
				}
				break
			}
		}
...
func TestAccApsaraStackKMSKey_basic(t *testing.T) {
	var v kms.KeyMetadata
	resourceId := "apsarastack_kms_key.default"
