...
func resourceApsaraStackRosStack() *schema.Resource {
	return &schema.Resource{
		Create: resourceApsaraStackRosStackCreate,
		Read:   resourceApsaraStackRosStackRead,
		Update: resourceApsaraStackRosStackUpdate,
		Delete: resourceApsaraStackRosStackDelete,
...
func resourceApsaraStackRosStackCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return WrapErrorf(err, DefaultErrorMsg, "apsarastack_ros_stack", action, ApsaraStackSdkGoERROR)
	}
	...
	return resourceApsaraStackRosStackRead(d, meta)
}
func resourceApsaraStackRosStackRead(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.ApsaraStackClient)
	rosService := RosService{client}
	object, err := rosService.DescribeRosStack(d.Id())
	if err != nil {
		if NotFoundError(err) {
			log.Printf("[DEBUG] Resource apsarastack_ros_stack rosService.DescribeRosStack Failed!!! %s", err)
			d.SetId("")
			return nil
		}
		return WrapError(err)
...
func resourceApsaraStackRosStackUpdate(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.ApsaraStackClient)
	rosService := RosService{client}
	var response map[string]interface{}
	d.Partial(true)
...
func resourceApsaraStackRosStackDelete(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.ApsaraStackClient)
	rosService := RosService{client}
	action := "DeleteStack"
	var response map[string]interface{}
