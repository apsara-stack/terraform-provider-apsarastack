...
func resourceApsaraStackLogMachineGroup() *schema.Resource {
	return &schema.Resource{
		Create: resourceApsaraStackLogMachineGroupCreate,
		Read:   resourceApsaraStackLogMachineGroupRead,
		Update: resourceApsaraStackLogMachineGroupUpdate,
		Delete: resourceApsaraStackLogMachineGroupDelete,
...
func resourceApsaraStackLogMachineGroupCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if err := resource.Retry(2*time.Minute, func() *resource.RetryError {
		...
		return WrapErrorf(err, DefaultErrorMsg, "apsarastack_log_machine_group", "CreateMachineGroup", ApsaraStackLogGoSdkERROR)
	}

	d.SetId(fmt.Sprintf("%s%s%s", d.Get("project").(string), COLON_SEPARATED, d.Get("name").(string)))

	return resourceApsaraStackLogMachineGroupRead(d, meta)
}

func resourceApsaraStackLogMachineGroupRead(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.ApsaraStackClient)
...
func resourceApsaraStackLogMachineGroupUpdate(d *schema.ResourceData, meta interface{}) error {
	if d.HasChange("identify_type") || d.HasChange("identify_list") || d.HasChange("topic") {
		...
		if err := resource.Retry(2*time.Minute, func() *resource.RetryError {
			...
			return WrapErrorf(err, DefaultErrorMsg, d.Id(), "UpdateMachineGroup", ApsaraStackLogGoSdkERROR)
		}
	}
...
func resourceApsaraStackLogMachineGroupDelete(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.ApsaraStackClient)
	logService := LogService{client}
	parts, err := ParseResourceId(d.Id(), 2)
	if err != nil {
