...
func init() {
	resource.AddTestSweepers("apsarastack_launch_template", &resource.Sweeper{
		Name: "apsarastack_launch_template",
		F:    testApsaraStackLaunchTemplate,
	})
}

func testApsaraStackLaunchTemplate(region string) error {
	...
	if err != nil {
		return WrapErrorf(err, DefaultErrorMsg, region, request.GetActionName(), ApsaraStackSdkGoERROR)
	}
...
func TestAccApsaraStackLaunchTemplateBasic(t *testing.T) {
	var v ecs.LaunchTemplateSet

	resourceId := "apsarastack_launch_template.default"
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				Config: testAccConfig(map[string]interface{}{
					"security_group_id": "${apsarastack_security_group.default.id}_change",
				}),
				Check: resource.ComposeTestCheckFunc(
					testAccCheck(map[string]string{
			...
			{
				Config: testAccConfig(map[string]interface{}{
					"instance_type": "${data.apsarastack_instance_types.default.instance_types.0.id}_change",
				}),
				Check: resource.ComposeTestCheckFunc(
					testAccCheck(map[string]string{
			...
			{
				Config: testAccConfig(map[string]interface{}{
					"image_id": "${data.apsarastack_images.default.images.0.id}_change",
				}),
				Check: resource.ComposeTestCheckFunc(
					testAccCheck(map[string]string{
						"image_id": CHECKSET,
...
func TestAccApsaraStackLaunchTemplateMulti(t *testing.T) {
	var v ecs.LaunchTemplateSet

	resourceId := "apsarastack_launch_template.default.4"
	ra := resourceAttrInit(resourceId, testAccLaunchTemplateCheckMap)
	serviceFunc := func() interface{} {
		return &EcsService{testAccProvider.Meta().(*connectivity.ApsaraStackClient)}
	}
