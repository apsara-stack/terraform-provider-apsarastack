...
func TestAccApsaraStackCrEESyncRulesDataSource(t *testing.T) {
	rand := acctest.RandIntRange(1000000, 9999999)
	name := fmt.Sprintf("tf-testacc-cr-ee-sr-%d", rand)
	resourceId := "data.apsarastack_cr_ee_sync_rules.default"
	...
	nameRegexConf := dataSourceTestAccConfig{
		existConfig: testAccConfig(map[string]interface{}{
			...
			"name_regex":  "${apsarastack_cr_ee_sync_rule.default.name}",
		}),
		fakeConfig: testAccConfig(map[string]interface{}{
			"instance_id": "${apsarastack_cr_ee_namespace.source_ns.instance_id}",
			"name_regex":  "${apsarastack_cr_ee_sync_rule.default.name}-fake",
		}),
	}
	...
	idsConf := dataSourceTestAccConfig{
		existConfig: testAccConfig(map[string]interface{}{
			"instance_id": "${apsarastack_cr_ee_namespace.source_ns.instance_id}",
			"ids":         []string{"${apsarastack_cr_ee_sync_rule.default.rule_id}"},
		}),
		fakeConfig: testAccConfig(map[string]interface{}{
			"instance_id": "${apsarastack_cr_ee_namespace.source_ns.instance_id}",
			"ids":         []string{"${apsarastack_cr_ee_sync_rule.default.rule_id}-fake"},
		}),
	}
	...
	namespaceConf := dataSourceTestAccConfig{
		...
		fakeConfig: testAccConfig(map[string]interface{}{
			"instance_id":    "${apsarastack_cr_ee_namespace.source_ns.instance_id}",
			"namespace_name": "${apsarastack_cr_ee_namespace.source_ns.name}-fake",
		}),
	}
	...
	repoConf := dataSourceTestAccConfig{
		existConfig: testAccConfig(map[string]interface{}{
			"instance_id": "${apsarastack_cr_ee_namespace.source_ns.instance_id}",
			"repo_name":   "${apsarastack_cr_ee_repo.source_repo.name}",
		}),
		fakeConfig: testAccConfig(map[string]interface{}{
			"instance_id": "${apsarastack_cr_ee_namespace.source_ns.instance_id}",
			"repo_name":   "${apsarastack_cr_ee_repo.source_repo.name}-fake",
		}),
	}
	...
	targetInstanceIdConf := dataSourceTestAccConfig{
		...
		fakeConfig: testAccConfig(map[string]interface{}{
			"instance_id":        "${apsarastack_cr_ee_namespace.source_ns.instance_id}",
			"target_instance_id": "${apsarastack_cr_ee_namespace.target_ns.instance_id}-fake",
		}),
	}
...
resource "apsarastack_cr_ee_sync_rule" "default" {
	...
	tag_filter = ".*"
	repo_name = "${apsarastack_cr_ee_repo.source_repo.name}"
	target_repo_name = "${apsarastack_cr_ee_repo.target_repo.name}"
}
`
