...
func testSweepCRNamespace(region string) error {
	...
	req.QueryParams = map[string]string{"AccessKeySecret": client.SecretKey, "Product": "cr", "Department": client.Department, "ResourceGroup": client.ResourceGroup}
	...
	if err != nil {
		log.Printf("[ERROR] %s ", WrapErrorf(err, DefaultErrorMsg, "apsarastack_cr_namespace", req.GetActionName(), ApsaraStackSdkGoERROR))
		return nil
	}
	...
	for _, n := range ns {
		err := resource.Retry(5*time.Minute, func() *resource.RetryError {
			...
			if err != nil {
				...
				return resource.RetryableError(WrapErrorf(err, DefaultErrorMsg, n, req.GetActionName(), ApsaraStackSdkGoERROR))
			}
...
func TestAccApsaraStackCRNamespace_Multi(t *testing.T) {
	var v *cr.GetNamespaceResponse
	resourceId := "apsarastack_cr_namespace.default.4"
	ra := resourceAttrInit(resourceId, crNamespaceMap)
	serviceFunc := func() interface{} {
		return &CrService{testAccProvider.Meta().(*connectivity.ApsaraStackClient)}
	}
