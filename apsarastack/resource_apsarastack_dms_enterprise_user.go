...
func resourceApsaraStackDmsEnterpriseUser() *schema.Resource {
	return &schema.Resource{
		Create: resourceApsaraStackDmsEnterpriseUserCreate,
		Read:   resourceApsaraStackDmsEnterpriseUserRead,
		Update: resourceApsaraStackDmsEnterpriseUserUpdate,
		Delete: resourceApsaraStackDmsEnterpriseUserDelete,
...
func resourceApsaraStackDmsEnterpriseUserCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return WrapErrorf(err, DefaultErrorMsg, "apsarastack_dms_enterprise_user", action, ApsaraStackSdkGoERROR)
	}

	d.SetId(fmt.Sprint(request["Uid"]))

	return resourceApsaraStackDmsEnterpriseUserUpdate(d, meta)
}
func resourceApsaraStackDmsEnterpriseUserRead(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.ApsaraStackClient)
	dms_enterpriseService := Dms_enterpriseService{client}
	object, err := dms_enterpriseService.DescribeDmsEnterpriseUser(d.Id())
	if err != nil {
		if NotFoundError(err) {
			log.Printf("[DEBUG] Resource apsarastack_dms_enterprise_user dms_enterpriseService.DescribeDmsEnterpriseUser Failed!!! %s", err)
			d.SetId("")
			return nil
		}
		return WrapError(err)
...
func resourceApsaraStackDmsEnterpriseUserUpdate(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.ApsaraStackClient)
	dms_enterpriseService := Dms_enterpriseService{client}
	var response map[string]interface{}
	d.Partial(true)
	...
	d.Partial(false)
	return resourceApsaraStackDmsEnterpriseUserRead(d, meta)
}
func resourceApsaraStackDmsEnterpriseUserDelete(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.ApsaraStackClient)
	action := "DeleteUser"
	var response map[string]interface{}
	conn, err := client.NewDmsenterpriseClient()
