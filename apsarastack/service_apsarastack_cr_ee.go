...
func (c *CrService) ListCrEEInstances(pageNo int, pageSize int) (*cr_ee.ListInstanceResponse, error) {
	...
	if err != nil {
		return response, WrapErrorf(err, DataDefaultErrorMsg, "ListInstance", action, ApsaraStackSdkGoERROR)
	}
	addDebug(action, raw, request.RpcRequest, request)

	response, _ = raw.(*cr_ee.ListInstanceResponse)
	if !response.ListInstanceIsSuccess {
		return response, WrapErrorf(errors.New(response.Code), DataDefaultErrorMsg, "ListInstance", action, ApsaraStackSdkGoERROR)
	}
	return response, nil
}
...
func (c *CrService) DescribeCrEEInstance(instanceId string) (*cr_ee.GetInstanceResponse, error) {
	...
	if err != nil {
		...
		return response, WrapErrorf(err, DefaultErrorMsg, resource, action, ApsaraStackSdkGoERROR)
	}
	addDebug(action, raw, request.RpcRequest, request)
...
func (c *CrService) ListCrEENamespaces(instanceId string, pageNo int, pageSize int) (*cr_ee.ListNamespaceResponse, error) {
	...
	if err != nil {
		return response, WrapErrorf(err, DataDefaultErrorMsg, resource, action, ApsaraStackSdkGoERROR)
	}
	addDebug(action, raw, request.RpcRequest, request)

	response, _ = raw.(*cr_ee.ListNamespaceResponse)
	if !response.ListNamespaceIsSuccess {
		return response, WrapErrorf(errors.New(response.Code), DataDefaultErrorMsg, resource, action, ApsaraStackSdkGoERROR)
	}
	return response, nil
}
...
func (c *CrService) DescribeCrEESyncRule(id string) (*cr_ee.SyncRulesItem, error) {
	...
	for {
		...
		if len(response.SyncRules) < PageSizeLarge {
			return nil, WrapErrorf(errors.New("sync rule not found"), NotFoundMsg, ApsaraStackSdkGoERROR)
		}
...
func (c *CrService) wrapCrServiceError(resource string, action string, code string) error {
	...
	case "INSTANCE_NOT_EXIST", "NAMESPACE_NOT_EXIST", "REPO_NOT_EXIST":
		return WrapErrorf(errors.New(code), NotFoundMsg, ApsaraStackSdkGoERROR)
	default:
		return WrapErrorf(errors.New(code), DefaultErrorMsg, resource, action, ApsaraStackSdkGoERROR)
	}
}
