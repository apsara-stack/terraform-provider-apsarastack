...
func TestAccApsaraStackSecurityGroupRuleBasic(t *testing.T) {
	var v ecs.Permission
	resourceId := "apsarastack_security_group_rule.default"
...
func TestAccApsaraStackSecurityGroupEgressRule(t *testing.T) {
	var v ecs.Permission
	resourceId := "apsarastack_security_group_rule.default"
...
func TestAccApsaraStackSecurityGroupRuleMulti(t *testing.T) {
	var v ecs.Permission
	resourceId := "apsarastack_security_group_rule.default.2"
	ra := resourceAttrInit(resourceId, testAccCheckSecurityGroupRuleBasicMap)
	serviceFunc := func() interface{} {
		return &EcsService{testAccProvider.Meta().(*connectivity.ApsaraStackClient)}
	}
...
resource "apsarastack_security_group_rule" "default" {
  ...
  port_range = "22/22"
  priority = 100
  security_group_id = "${apsarastack_security_group.default.0.id}"
  source_security_group_id = "${apsarastack_security_group.default.1.id}"
  description = "abc"
}
`
...
func testAccCheckSecurityGroupRuleDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "apsarastack_security_group_rule" {
			continue
		}
