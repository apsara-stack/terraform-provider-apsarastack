...
func (s *SlbService) DescribeSlb(id string) (*slb.DescribeLoadBalancerAttributeResponse, error) {
	...
	if err != nil {
		if IsExpectedErrors(err, []string{"InvalidLoadBalancerId.NotFound"}) {
			err = WrapErrorf(Error(GetNotFoundMessage("Slb", id)), NotFoundMsg, ApsaraStackSdkGoERROR)
		} else {
...
func (s *SlbService) DescribeSlbMasterSlaveServerGroup(id string) (*slb.DescribeMasterSlaveServerGroupAttributeResponse, error) {
	...
	if err != nil {
		...
		return response, WrapErrorf(err, DefaultDebugMsg, id, request.GetActionName(), ApsaraStackSdkGoERROR)
	}
	addDebug(request.GetActionName(), raw, request.RpcRequest, request)
...
func (s *SlbService) DescribeSlbBackendServer(id string) (*slb.DescribeLoadBalancerAttributeResponse, error) {
	...
	if err != nil {
		if IsExpectedErrors(err, []string{"InvalidLoadBalancerId.NotFound"}) {
			err = WrapErrorf(Error(GetNotFoundMessage("SlbBackendServers", id)), NotFoundMsg, ApsaraStackSdkGoERROR)
		} else {
			err = WrapErrorf(err, DefaultErrorMsg, id, request.GetActionName(), ApsaraStackSdkGoERROR)
		}
		return response, err
...
func (s *SlbService) DescribeSlbCACertificate(id string) (*slb.CACertificate, error) {
	...
	if err != nil {
		return certificate, WrapErrorf(err, DefaultErrorMsg, id, request.GetActionName(), ApsaraStackSdkGoERROR)
	}
	addDebug(request.GetActionName(), raw, request.RpcRequest, request)
...
func (s *SlbService) DescribeDomainExtensionAttribute(domainExtensionId string) (*slb.DescribeDomainExtensionAttributeResponse, error) {
	...
	err = resource.Retry(5*time.Minute, func() *resource.RetryError {
		...
		if err != nil {
			if IsExpectedErrors(err, []string{ApsaraStackGoClientFailure, "ServiceUnavailable", Throttling}) {
	...
	if err != nil {
		...
		return response, WrapErrorf(err, DefaultErrorMsg, domainExtensionId, request.GetActionName(), ApsaraStackSdkGoERROR)
	}
...
func (s *SlbService) DescribeAccessLogsDownloadAttribute(logs_type string, load_balancer_id string) (logsattr []interface{}, err error) {
	...
	for {
		...
		if err != nil {
			return nil, WrapErrorf(err, DefaultErrorMsg, "apsarastack_slb", request.GetActionName(), ApsaraStackSdkGoERROR)
		}
		addDebug(request.GetActionName(), raw, request, request.QueryParams)
		response, _ := raw.(*responses.CommonResponse)
		if !response.IsSuccess() {
