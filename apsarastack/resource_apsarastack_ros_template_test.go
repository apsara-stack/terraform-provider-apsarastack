...
func init() {
	resource.AddTestSweepers(
		"apsarastack_ros_template",
		&resource.Sweeper{
			Name: "apsarastack_ros_template",
...
func testSweepRosTemplate(region string) error {
	...
	for {
		...
		if err != nil {
			return WrapErrorf(err, DataDefaultErrorMsg, "apsarastack_ros_template", action, ApsaraStackSdkGoERROR)
		}
...
func TestAccApsaraStackRosTemplate_basic(t *testing.T) {
	var v map[string]interface{}
	resourceId := "apsarastack_ros_template.default"
	ra := resourceAttrInit(resourceId, ApsaraStackRosTemplateMap)
	rc := resourceCheckInitWithDescribeMethod(resourceId, &v, func() interface{} {
		return &RosService{testAccProvider.Meta().(*connectivity.ApsaraStackClient)}
	}, "DescribeRosTemplate")
	rac := resourceAttrCheckInit(rc, ra)
	testAccCheck := rac.resourceAttrMapUpdateSet()
	rand := acctest.RandIntRange(10000, 99999)
	name := fmt.Sprintf("tf-testAcc%sApsaraStackRosTemplate%d", defaultRegionToTest, rand)
	testAccConfig := resourceTestAccConfigFunc(resourceId, name, ApsaraStackRosTemplateBasicDependence)
...
var ApsaraStackRosTemplateMap = map[string]string{}

func ApsaraStackRosTemplateBasicDependence(name string) string {
	return ""
}
