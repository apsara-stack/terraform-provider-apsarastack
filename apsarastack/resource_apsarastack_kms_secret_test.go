...
func init() {
	resource.AddTestSweepers("apsarastack_kms_secret", &resource.Sweeper{
		Name: "apsarastack_kms_secret",
...
func TestAccApsaraStackKmsSecret_Basic(t *testing.T) {
	var v kms.DescribeSecretResponse

	resourceId := "apsarastack_kms_secret.default"
...
func TestAccApsaraStackKmsSecret_WithKey(t *testing.T) {
	var v kms.DescribeSecretResponse
		...
		Steps: []resource.TestStep{
			{
				Config: testAccConfig(map[string]interface{}{
					...
					"version_id":                    "00001",
					"force_delete_without_recovery": "true",
					"encryption_key_id":             "${apsarastack_kms_key.default.id}",
				}),
				Check: resource.ComposeTestCheckFunc(
					testAccCheck(map[string]string{
						"secret_data": name,
