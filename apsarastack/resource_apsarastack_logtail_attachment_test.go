...
func TestAccApsaraStackLogtailAttachmentBasic(t *testing.T) {
	var v string
	resourceId := "apsarastack_logtail_attachment.default"
		...
		Steps: []resource.TestStep{
			{
				Config: testAccConfig(map[string]interface{}{
					"project":             "${apsarastack_log_project.default.name}",
					"logtail_config_name": "${apsarastack_logtail_config.default.name}",
					"machine_group_name":  "${apsarastack_log_machine_group.default.name}",
				}),
...
func TestAccApsaraStackLogtailAttachmentMultipleGroup(t *testing.T) {
	var v string
	resourceId := "apsarastack_logtail_attachment.default.1"
	ra := resourceAttrInit(resourceId, logtailAttachmentMap)
	serviceFunc := func() interface{} {
		return &LogService{testAccProvider.Meta().(*connectivity.ApsaraStackClient)}
	}
		...
		Steps: []resource.TestStep{
			{
				Config: testAccConfig(map[string]interface{}{
					"project":             "${apsarastack_log_project.default.name}",
					"logtail_config_name": "${apsarastack_logtail_config.default.name}",
					"machine_group_name":  "${element(apsarastack_log_machine_group.default.*.name,count.index)}",
					"count":               "2",
				}),
				Check: resource.ComposeTestCheckFunc(
					testAccCheck(nil),
...
func TestAccApsaraStackLogtailAttachmentMultipleConfig(t *testing.T) {
	var v string
	resourceId := "apsarastack_logtail_attachment.default.1"
	ra := resourceAttrInit(resourceId, logtailAttachmentMap)
	serviceFunc := func() interface{} {
		...
		Steps: []resource.TestStep{
			{
				Config: testAccConfig(map[string]interface{}{
					"project":             "${apsarastack_log_project.default.name}",
					"logtail_config_name": "${element(apsarastack_logtail_config.default.*.name, count.index)}",
					"machine_group_name":  "${apsarastack_log_machine_group.default.name}",
					"count":               "2",
				}),
				Check: resource.ComposeTestCheckFunc(
...
resource "apsarastack_log_project" "default"{
	name = "${var.name}"
	description = "create by terraform"
}
resource "apsarastack_log_store" "default"{
  	project = "${apsarastack_log_project.default.name}"
  	name = "${var.name}"
...
resource "apsarastack_log_machine_group" "default" {
	    project = "${apsarastack_log_project.default.name}"
	    name = "${var.name}"
	    topic = "terraform"
	    identify_list = ["10.0.0.1", "10.0.0.3", "10.0.0.2"]
}
resource "apsarastack_logtail_config" "default"{
	project = "${apsarastack_log_project.default.name}"
  	logstore = "${apsarastack_log_store.default.name}"
  	input_type = "file"
