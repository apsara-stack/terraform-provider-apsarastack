...
func resourceApsarastackCmsAlarmContact() *schema.Resource {
	return &schema.Resource{
		Create: resourceApsarastackCmsAlarmContactCreate,
		Read:   resourceApsarastackCmsAlarmContactRead,
		Update: resourceApsarastackCmsAlarmContactUpdate,
		Delete: resourceApsarastackCmsAlarmContactDelete,
...
func resourceApsarastackCmsAlarmContactCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return WrapErrorf(err, DefaultErrorMsg, "apsarastack_cms_alarm_contact", request.GetActionName(), ApsaraStackSdkGoERROR)
	}
	...
	return resourceApsarastackCmsAlarmContactRead(d, meta)
}
func resourceApsarastackCmsAlarmContactRead(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.ApsaraStackClient)
	cmsService := CmsService{client}
	object, err := cmsService.DescribeCmsAlarmContact(d.Id())
	if err != nil {
		if NotFoundError(err) {
			log.Printf("[DEBUG] Resource apsarastack_cloud_monitor_service_alarm_contact cmsService.DescribeCmsAlarmContact Failed!!! %s", err)
			d.SetId("")
			return nil
		}
...
func resourceApsarastackCmsAlarmContactUpdate(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.ApsaraStackClient)
	update := false
	request := cms.CreatePutContactRequest()
...
func resourceApsarastackCmsAlarmContactDelete(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.ApsaraStackClient)
