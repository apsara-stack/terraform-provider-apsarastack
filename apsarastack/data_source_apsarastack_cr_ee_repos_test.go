...
func TestAccApsaraStackCrEEReposDataSource(t *testing.T) {
	...
	nameRegexConf := dataSourceTestAccConfig{
		existConfig: testAccConfig(map[string]interface{}{
			...
			"namespace":   "${apsarastack_cr_ee_namespace.default.name}",
			"name_regex":  "${apsarastack_cr_ee_repo.default.name}",
		}),
		fakeConfig: testAccConfig(map[string]interface{}{
			"instance_id": "${data.apsarastack_cr_ee_instances.default.ids.0}",
			"namespace":   "${apsarastack_cr_ee_namespace.default.name}",
			"name_regex":  "${apsarastack_cr_ee_repo.default.name}-fake",
		}),
	}
	...
	idsConf := dataSourceTestAccConfig{
		existConfig: testAccConfig(map[string]interface{}{
			"instance_id": "${data.apsarastack_cr_ee_instances.default.ids.0}",
			"namespace":   "${apsarastack_cr_ee_namespace.default.name}",
			"ids":         []string{"${apsarastack_cr_ee_repo.default.repo_id}"},
		}),
		fakeConfig: testAccConfig(map[string]interface{}{
			"instance_id": "${data.apsarastack_cr_ee_instances.default.ids.0}",
			"namespace":   "${apsarastack_cr_ee_namespace.default.name}",
			"ids":         []string{"${apsarastack_cr_ee_repo.default.repo_id}-fake"},
		}),
	}
...
func dataSourceCrEEReposConfigDependence(name string) string {
	...
	resource "apsarastack_cr_ee_repo" "default" {
		instance_id = "${data.apsarastack_cr_ee_instances.default.ids.0}"
		namespace = "${apsarastack_cr_ee_namespace.default.name}"
		name = "${var.name}"
