...
func init() {
	resource.AddTestSweepers("apsarastack_cms_alarm_contact_group", &resource.Sweeper{
		Name: "apsarastack_cms_alarm_contact_group",
...
func TestAccApsarastackCmsAlarmContactGroup_basic(t *testing.T) {
	var v cms.ContactGroup
	resourceId := "apsarastack_cms_alarm_contact_group.default"
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				Config: testAccConfig(map[string]interface{}{
					"contacts": []string{"${apsarastack_cms_alarm_contact.default.id}"},
				}),
				Check: resource.ComposeTestCheckFunc(
					testAccCheck(map[string]string{
						"contacts.#": "1",
			...
			},
			{
				Config: testAccConfig(map[string]interface{}{
					...
					"contacts":          []string{"${apsarastack_cms_alarm_contact.default.id}", "${apsarastack_cms_alarm_contact.default0.id}"},
...
resource "apsarastack_cms_alarm_contact" "default0" {
