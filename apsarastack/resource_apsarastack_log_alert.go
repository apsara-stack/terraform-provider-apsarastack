...
func resourceApsaraStackLogAlert() *schema.Resource {
	return &schema.Resource{
		Create: resourceApsaraStackLogAlertCreate,
		Read:   resourceApsaraStackLogAlertRead,
		Update: resourceApsaraStackLogAlertUpdate,
		Delete: resourceApsaraStackLogAlertDelete,
...
func resourceApsaraStackLogAlertCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if err := resource.Retry(2*time.Minute, func() *resource.RetryError {
		...
		return WrapErrorf(err, DefaultErrorMsg, "apsarastack_log_alert", "CreateLogstoreAlert", ApsaraStackSdkGoERROR)
	}

	d.SetId(fmt.Sprintf("%s%s%s", project_name, COLON_SEPARATED, alert_name))
	return resourceApsaraStackLogAlertRead(d, meta)
}

func resourceApsaraStackLogAlertRead(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.ApsaraStackClient)
...
func resourceApsaraStackLogAlertUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if err := resource.Retry(2*time.Minute, func() *resource.RetryError {
		...
		return WrapErrorf(err, DefaultErrorMsg, d.Id(), "UpdateAlert", ApsaraStackSdkGoERROR)
	}

	return resourceApsaraStackLogAlertRead(d, meta)
}

func resourceApsaraStackLogAlertDelete(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.ApsaraStackClient)
	logService := LogService{client}
	parts, err := ParseResourceId(d.Id(), 2)
	if err != nil {
	...
	if err != nil {
		return WrapErrorf(err, DefaultErrorMsg, "apsarastack_log_alert", "DeleteAlert", ApsaraStackSdkGoERROR)
	}
