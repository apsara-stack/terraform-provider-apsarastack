...
func resourceApsaraStackDmsEnterpriseInstance() *schema.Resource {
	return &schema.Resource{
		Create: resourceApsaraStackDmsEnterpriseInstanceCreate,
		Read:   resourceApsaraStackDmsEnterpriseInstanceRead,
		Update: resourceApsaraStackDmsEnterpriseInstanceUpdate,
		Delete: resourceApsaraStackDmsEnterpriseInstanceDelete,
...
func resourceApsaraStackDmsEnterpriseInstanceCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return WrapErrorf(err, DefaultErrorMsg, "apsarastack_dms_enterprise_instance", action, ApsaraStackSdkGoERROR)
	}

	d.SetId(fmt.Sprint(request["Host"], ":", request["Port"]))

	return resourceApsaraStackDmsEnterpriseInstanceUpdate(d, meta)
}
func resourceApsaraStackDmsEnterpriseInstanceRead(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.ApsaraStackClient)
	...
	if err != nil {
		if NotFoundError(err) {
			log.Printf("[DEBUG] Resource apsarastack_dms_enterprise_instance dms_enterpriseService.DescribeDmsEnterpriseInstance Failed!!! %s", err)
			d.SetId("")
			return nil
		}
		return WrapError(err)
...
func resourceApsaraStackDmsEnterpriseInstanceUpdate(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.ApsaraStackClient)
	var response map[string]interface{}
	parts, err := ParseResourceId(d.Id(), 2)
	if err != nil {
	...
	return resourceApsaraStackDmsEnterpriseInstanceRead(d, meta)
}
func resourceApsaraStackDmsEnterpriseInstanceDelete(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.ApsaraStackClient)
	parts, err := ParseResourceId(d.Id(), 2)
	if err != nil {
		return WrapError(err)
