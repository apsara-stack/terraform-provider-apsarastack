...
func init() {
	resource.AddTestSweepers(
		"apsarastack_ros_stack",
		&resource.Sweeper{
			Name: "apsarastack_ros_stack",
			F:    testSweepRosStack,
		})
}
...
func testSweepRosStack(region string) error {
	...
	for {
		...
		if err != nil {
			return WrapErrorf(err, DataDefaultErrorMsg, "apsarastack_ros_stack", action, ApsaraStackSdkGoERROR)
		}
...
func TestAccApsaraStackRosStack_basic(t *testing.T) {
	var v map[string]interface{}
	resourceId := "apsarastack_ros_stack.default"
	ra := resourceAttrInit(resourceId, ApsaraStackRosStackMap)
	...
	name := fmt.Sprintf("tf-testAcc%sApsaraStackRosStack%d", defaultRegionToTest, rand)
	testAccConfig := resourceTestAccConfigFunc(resourceId, name, ApsaraStackRosStackBasicDependence)
...
var ApsaraStackRosStackMap = map[string]string{
	"deletion_protection": "Disabled",
	"status":              CHECKSET,
}

func ApsaraStackRosStackBasicDependence(name string) string {
	return ""
}
