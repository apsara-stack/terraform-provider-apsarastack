...
func setTags(client *connectivity.ApsaraStackClient, resourceType TagResourceType, d *schema.ResourceData) error {
	if d.HasChange("tags") {
...
func setCdnTags(client *connectivity.ApsaraStackClient, resourceType TagResourceType, d *schema.ResourceData) error {
	if d.HasChange("tags") {
		oraw, nraw := d.GetChange("tags")
...
func setVolumeTags(client *connectivity.ApsaraStackClient, resourceType TagResourceType, d *schema.ResourceData) error {
...
func updateTags(client *connectivity.ApsaraStackClient, ids []string, resourceType TagResourceType, oraw, nraw interface{}) error {
	...
	if len(remove) > 0 {
		...
		if err != nil {
			return WrapErrorf(err, DefaultErrorMsg, ids, request.GetActionName(), ApsaraStackSdkGoERROR)
		}
	}
...
func updateCdnTags(client *connectivity.ApsaraStackClient, ids []string, resourceType TagResourceType, oraw, nraw interface{}) error {
	o := oraw.(map[string]interface{})
	n := nraw.(map[string]interface{})
	create, remove := diffTags(tagsFromMap(o), tagsFromMap(n))
