...
func resourceApsaraStackLogStoreIndex() *schema.Resource {
	return &schema.Resource{
		Create: resourceApsaraStackLogStoreIndexCreate,
		Read:   resourceApsaraStackLogStoreIndexRead,
		Update: resourceApsaraStackLogStoreIndexUpdate,
		Delete: resourceApsaraStackLogStoreIndexDelete,
...
func resourceApsaraStackLogStoreIndexCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if err := resource.Retry(2*time.Minute, func() *resource.RetryError {
		...
		if err != nil {
			if IsExpectedErrors(err, []string{LogClientTimeout}) {
				...
				return resource.RetryableError(WrapErrorf(err, DefaultErrorMsg, "apsarastack_log_store", "GetIndex", ApsaraStackLogGoSdkERROR))
			}
			if !IsExpectedErrors(err, []string{"IndexConfigNotExist"}) {
				return resource.NonRetryableError(WrapErrorf(err, DefaultErrorMsg, "apsarastack_log_store", "GetIndex", ApsaraStackLogGoSdkERROR))
			}
		}
		if raw != nil {
	...
	if err := resource.Retry(2*time.Minute, func() *resource.RetryError {
		...
		return WrapErrorf(err, DefaultErrorMsg, "apsarastack_log_store_index", "CreateIndex", ApsaraStackLogGoSdkERROR)
	}

	d.SetId(fmt.Sprintf("%s%s%s", project, COLON_SEPARATED, store.Name))

	return resourceApsaraStackLogStoreIndexRead(d, meta)
}

func resourceApsaraStackLogStoreIndexRead(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.ApsaraStackClient)
	logService := LogService{client}
...
func resourceApsaraStackLogStoreIndexUpdate(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.ApsaraStackClient)
	...
	if update {
		...
		if err := resource.Retry(2*time.Minute, func() *resource.RetryError {
			...
			return WrapErrorf(err, DefaultErrorMsg, d.Id(), "UpdateIndex", ApsaraStackLogGoSdkERROR)
		}
	}
...
func resourceApsaraStackLogStoreIndexDelete(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.ApsaraStackClient)
	logService := LogService{client}
	...
	if err := resource.Retry(2*time.Minute, func() *resource.RetryError {
		...
		return WrapErrorf(err, DefaultErrorMsg, d.Id(), "DeleteIndex", ApsaraStackLogGoSdkERROR)
	}
	return nil
}
