...
func init() {
	resource.AddTestSweepers("apsarastack_security_group", &resource.Sweeper{
		Name: "apsarastack_security_group",
...
func testAccCheckSecurityGroupDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "apsarastack_security_group" {
			continue
		}
...
func TestAccApsaraStackSecurityGroupBasic(t *testing.T) {
	var v ecs.DescribeSecurityGroupAttributeResponse
	resourceId := "apsarastack_security_group.default"
...
func TestAccApsaraStackSecurityGroupMulti(t *testing.T) {
	var v ecs.DescribeSecurityGroupAttributeResponse
	resourceId := "apsarastack_security_group.default.9"
	ra := resourceAttrInit(resourceId, testAccCheckSecurityBasicMap)
	serviceFunc := func() interface{} {
		return &EcsService{testAccProvider.Meta().(*connectivity.ApsaraStackClient)}
	}
