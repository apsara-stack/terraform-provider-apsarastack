...
func (s *RamService) JudgeRolePolicyPrincipal(roleName string) error {
	...
	if err != nil {
		return WrapErrorf(err, DefaultErrorMsg, roleName, request.GetActionName(), ApsaraStackSdkGoERROR)
	}
...
func (s *RamService) DescribeRamUser(id string) (*ram.UserInGetUser, error) {
	...
	for {
		...
		if err != nil {
			return user, WrapErrorf(err, DefaultErrorMsg, id, listUsersRequest.GetActionName(), ApsaraStackSdkGoERROR)
		}
	...
	if err != nil {
		if IsExpectedErrors(err, []string{"EntityNotExist.User"}) {
			return user, WrapErrorf(err, NotFoundMsg, ApsaraStackSdkGoERROR)
		}
		return user, WrapErrorf(err, DefaultErrorMsg, id, getUserRequest.GetActionName(), ApsaraStackSdkGoERROR)
	}
...
func (s *RamService) DescribeRamAccessKey(id, userName string) (*ram.AccessKeyInListAccessKeys, error) {
	...
	if err != nil {
		if IsExpectedErrors(err, []string{"EntityNotExist"}) {
			return key, WrapErrorf(Error(GetNotFoundMessage("RamAccessKey", id)), NotFoundMsg, ApsaraStackSdkGoERROR)
		}
		return key, WrapErrorf(err, DefaultErrorMsg, id, request.GetActionName(), ApsaraStackSdkGoERROR)
	}
	addDebug(request.GetActionName(), raw, request.RpcRequest, request)
