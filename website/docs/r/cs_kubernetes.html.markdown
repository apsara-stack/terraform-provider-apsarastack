...
* `is_enterprise_security_group` -(Optional) Specifies whether an advanced security group is automatically created. If `is_enterprise_security_group` is "false" you must set the `security_group_id` parameter.
* `security_group_id` - (Optional) The ID of the security group to which the ECSs in the cluster belong.
